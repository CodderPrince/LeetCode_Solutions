...
public:
    int peopleAwareOfSecret(int n, int delay, int forget) {
        ...
        for (int day = 1; day <= n; ++day) {
            ...
            deltaKnowers[day + forget] =
                (deltaKnowers[day + forget] - newSpreaders[day] + MOD) % MOD;

            int startSpreadingDay = day + delay;
            while (startSpreadingDay < day + forget) {

                newSpreaders[startSpreadingDay] =
                    (newSpreaders[startSpreadingDay] + newSpreaders[day]) % MOD;
                ++startSpreadingDay;
            }
        }
